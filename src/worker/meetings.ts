...
const AUDIO_CLEANUP_INTERVAL_MS = Math.max(
  60_000,
  parseInt(process.env.MEETING_AUDIO_CLEANUP_INTERVAL_MS || '900000', 10)
)

let nextAudioCleanupAt = 0
let audioCleanupPromise: Promise<void> | null = null
...
async function maybeRunAudioCleanup(workerName: string) {
  if (workerName !== '1') return
  ...
  const now = Date.now()
  if (now < nextAudioCleanupAt) return

  audioCleanupPromise = (async () => {
    const supabase = createAdminClient()
    const result = await cleanupCompletedMeetingAudio(supabase)
    nextAudioCleanupAt = Date.now() + AUDIO_CLEANUP_INTERVAL_MS

    if (result.checked > 0 || result.failed > 0) {
      console.log(
        `[meeting-worker:${workerName}] audio cleanup checked=${result.checked} deleted=${result.deleted} failed=${result.failed} retention_hours=${result.retentionHours}`
      )
    }
  })()
...
async function workerLoop(workerName: string) {
  for (;;) {
    try {
      await maybeRunAudioCleanup(workerName)
...
async function main() {
  console.log(
    `[meeting-worker] Starting with concurrency=${CONCURRENCY} audio_cleanup_interval_ms=${AUDIO_CLEANUP_INTERVAL_MS}`
  )
  await Promise.all(
